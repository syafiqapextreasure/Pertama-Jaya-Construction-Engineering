...
export const FloatingEnquiry: React.FC<FloatingEnquiryProps> = ({
    ...
    return (
      <div className="fixed bottom-6 right-6 z-30 pointer-events-auto">
        <button
          ...
          aria-label="Kembangkan butang pertanyaan projek"
        >
          <MessageSquare className="w-5 h-5" />
          <span className="hidden sm:inline">Tanya Projek</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <aside
      aria-label="Pertanyaan Projek Segera"
    ...
    >
      <div className="flex items-start justify-between gap-2 pb-2 mb-2 border-b border-slate-800">
        ...
        <button
          ...
          aria-label="Kecilkan bar pertanyaan projek"
          title="Kecilkan"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <p className="text-[13px] text-slate-300 mb-3 leading-snug">
        Bincangkan keperluan tapak pembinaan, saliran atau penyelenggaraan loji anda bersama kami.
      </p>
